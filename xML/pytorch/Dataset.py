...
## 추가
from albumentations.augmentations.geometric.transforms import PiecewiseAffine
        ...
        [
            ...
            OneOf(
                [
                    ...
                    # PiecewiseAffine(p=0.3), -> 이걸로 대체했는드ㅔ 계속 에러 나서 부득이하게 없애버림
                ],
                p=0.2,
            ),
            OneOf(
...
def load_data(folder, dataset="imagenetHVZ", masks=False, class_weights=None):
    ...
    elif dataset in ["wavect", "embryo", "hmc", "kromp", "rehistogan", "rehistogan_ft", "stainnet", "stainnet_ft"]:
        return load_embryo_dataset(data=dataset, folder=folder, masks=masks, class_weights=class_weights)
...
## 추가
def load_embryo_dataset(data, folder, masks=False, class_weights=None):
        ...
        for cls in classes:
            ...
            for fname in os.listdir(cls_dir):
                if fname.lower().endswith((".png")):
                    records.append({
                        "imageID": os.path.join(cls_dir, fname),
                        "label": label_map[cls],
                        "split": split
                    })
    ...
    if df.empty:
        raise RuntimeError(f"No images found in {folder} (expected train/val/test subdirs)")
    ...
    # 4) split 별로 DataFrame 분리
    tr_df   = df[df.split == "train"].drop(columns=["split"]).reset_index(drop=True)
    ...
    print(f"[{data}] train/val/test = {len(tr_df)}/{len(val_df)}/{len(test_df)}  classes={classes}")
    return tr_df, val_df, test_df, weights, classes