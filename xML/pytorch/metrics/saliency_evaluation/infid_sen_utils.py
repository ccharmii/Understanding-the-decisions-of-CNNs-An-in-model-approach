## metrics/saliency_evaluation/infid_sen_utils.py
...
def get_exp_infid(image, model, exp, label, pdt, binary_I, pert):
    ...
    num = 50  ## 100에서 수정 (메모리 에러)

    exp = np.squeeze(exp)
    exp_copy = np.reshape(np.copy(exp), -1)
    image_copy = np.tile(np.reshape(np.copy(image.cpu()), (1, 3, 224 * 224)), [num, 1, 1])
...
def get_exp_sens(X, model, expl, yy, sen_r, sen_N, norm):
    ...
    for _ in range(sen_N):
        # 1) 노이즈 추가
        noise = sample_eps_Inf(X.cpu().numpy(), sen_r, 1)
        X_noisy = X + torch.from_numpy(noise).float().to(X.device)

        # 2) 노이즈 입력에 대한 explanation map 생성
        with torch.no_grad():
            # 예: 모델이 (logits, expl_map) tuple을 반환한다면
            _, expl_eps_tensor = model(X_noisy.unsqueeze(0))
        expl_eps = expl_eps_tensor.squeeze().cpu().numpy()

        # 3) 원본 expl (uint8) 와 비교
...
def evaluate_infid_sen(loader, model,
    ...
    for i, batch in enumerate(loader):
        ...
        X_all = batch["image"].cuda()    # (B,3,224,224)
