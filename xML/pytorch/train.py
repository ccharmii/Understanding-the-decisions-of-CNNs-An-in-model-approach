...
parser.add_argument(
    "--early_delta",
    type=float,  ## 수정: int였는데 float으로 수정
    default=1e-4,
...
def safe_torch_load(path, device):
    """PyTorch 2.6 호환: allowlist 추가 + weights_only 폴백 로더"""
    try:
        # 신뢰된 체크포인트라는 전제하에 numpy 객체 허용
        torch.serialization.add_safe_globals([np.core.multiarray.scalar, np.dtype])
    except Exception:
        pass

    # torch.load 시그니처에 weights_only가 있으면 True→False 폴백
...
val_loader = DataLoader(
    val_dataset,
    batch_size=args.batch_size,
    shuffle=False,  ## 수정: 원래 True였는데 False로 바꿈 (재현성을 위해)
    num_workers=args.num_workers,
    pin_memory=True,
    drop_last=False,
)
...
for phase in range(3):
    ...
    # Load best model
    ckpt = safe_torch_load(checkpoint_filename + "_best_loss.pt", device)
