...
class ExplainerClassifierCNN(nn.Module):
    ...
    def save_explanations(
        ...
        Keyword Arguments:
            ...
            only_explanations {bool} -- explanation만 추가적으로 저장할 수 있게 하는
        """
        ## only_explanations = True일 때 저장 폴더 생성
        os.makedirs(path, exist_ok=True)
        if only_explanations:
            expl_path = os.path.join(path, "explanations")
        ...
        print(f"\nSAVING EXPLANATIONS (phase {phase}){' [only explanation]' if only_explanations else ''}")
        ...
        with torch.no_grad():
            for batch_imgs, batch_labels, batch_names, _ in tqdm(
                ...
                batch_expls = self.explainer(batch_imgs)
                batch_probs = F.softmax(self.classifier(batch_imgs, batch_expls), dim=1)
                ...
                for idx, img in enumerate(batch_imgs):  # transverses the batch
                    ...
                    # --- 추가: only_explanations 모드이면 heatmap만 저장 ---
                    if only_explanations:
                        # expl: 2D numpy array (H×W)
                        expl_np = np.swapaxes(batch_expls[idx].squeeze().cpu().numpy(), 0, 1)
                        # 0–1 정규화
                        expl_norm = (expl_np - expl_np.min()) / (expl_np.max() - expl_np.min() + 1e-8)
                        fname = batch_names[idx].split("/")[-1]
                        out_file = os.path.join(expl_path, f"{timestamp}_phase{phase}_only_expl_{fname}")
                        # heatmap 저장
                        plt.imsave(out_file, expl_norm, cmap=cmap or "viridis", vmin=0, vmax=1)
    ...
    def forward(self, x, softmax=False):
        # 1) Explainer로부터 explanation map 생성
        expl = self.explainer(x)
        # 2) Classifier에 이미지+map을 넘겨 예측 logits 얻기
        logits = self.classifier(x, expl)
        # 3) softmax 플래그에 따라 확률 또는 원본 logits 반환
        if softmax:
